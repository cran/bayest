bayes.t.test <- function(n=10000,plot="all",firstComp,secondComp,hyperpars="wide",ci="0.95",burnin=n/2,sd="sd",q=0.1){
  ...
  if(sd=="var"){
    ...
    effectSizeSecC=(mu2SecC-mu1SecC)/(sqrt((sigma1SqSecC+sigma2SqSecC)/2))
    effectSizeThdC=(mu2ThdC-mu1ThdC)/(sqrt((sigma1SqThdC+sigma2SqThdC)/2))
    effectSizeFrtC=(mu2FrtC-mu1FrtC)/(sqrt((sigma1SqFrtC+sigma2SqFrtC)/2))
  }
  if(sd=="sd"){
    effectSize=(mu2-mu1)/(sqrt((sigma1Sq^2+sigma2Sq^2)/2))
    effectSizeSecC=(mu2SecC-mu1SecC)/(sqrt((sigma1SqSecC^2+sigma2SqSecC^2)/2))
    effectSizeThdC=(mu2ThdC-mu1ThdC)/(sqrt((sigma1SqThdC^2+sigma2SqThdC^2)/2))
    effectSizeFrtC=(mu2FrtC-mu1FrtC)/(sqrt((sigma1SqFrtC^2+sigma2SqFrtC^2)/2))
  }
  ...
  if(plot=="all"){
    ...
    mcmcMatrixMu1=matrix(0,nrow=length(mu1),ncol=4,byrow=FALSE)
    ...
    mcmcMatrixMu2=matrix(0,nrow=length(mu2),ncol=4,byrow=FALSE)
    ...
    mcmcMatrixSigma1Sq=matrix(0,nrow=length(sigma1Sq),ncol=4,byrow=FALSE)
    ...
    mcmcMatrixSigma2Sq=matrix(0,nrow=length(sigma2Sq),ncol=4,byrow=FALSE)
    ...
    plotpar<-par(mfrow=c(2,2))
    hist(diffOfMeans,freq=FALSE,main="Difference of means",col="cornflowerblue",border="white",xlab=expression(mu[2]-mu[1]))
    plot(diffOfMeans,ty="l",col="cornflowerblue",xlab=expression(mu[2]-mu[1]),ylab="")
    mcmcMatrixDiffOfMeans=matrix(0,nrow=length(diffOfMeans),ncol=4,byrow=FALSE)
    ...
    plotpar<-par(mfrow=c(2,2))
    if(sd=="var"){
      hist(diffOfVariances,freq=FALSE,main="Difference of variances",col="cornflowerblue",border="white",
           xlab=expression(sigma[2]^2-sigma[1]^2))
      plot(diffOfVariances,ty="l",col="cornflowerblue",xlab=expression(sigma[2]^2-sigma[1]^2),ylab="")
    }
    if(sd=="sd"){
      hist(diffOfVariances,freq=FALSE,main="Difference of standard deviations",col="cornflowerblue",border="white",
           xlab=expression(sigma[2]-sigma[1]))
      plot(diffOfVariances,ty="l",col="cornflowerblue",xlab=expression(sigma[2]-sigma[1]),ylab="")
    }
    mcmcMatrixDiffOfVariances=matrix(0,nrow=length(diffOfVariances),ncol=4,byrow=FALSE)
    ...
    plotpar<-par(mfrow=c(2,2))
    hist(effectSize,freq=FALSE,main="Effect size",col="cornflowerblue",border="white",xlab=expression(delta))
    plot(effectSize,ty="l",col="cornflowerblue",xlab=expression(delta),ylab="")
    mcmcMatrixEffectSize=matrix(0,nrow=length(effectSize),ncol=4,byrow=FALSE)
    ...
    hist(effectSize,freq=FALSE,col="cornflowerblue",border="white",xlab=expression(delta),main=mtext("Posterior distribution of Effect size "~delta, side=3,line=2.2,col="black",cex=1,font=2),xaxt="n")
    ...
    lines(x=c(getmode(effectSize),getmode(effectSize)),y=c(-0.025,0.025),type="l",lwd=2.5,col="blue")
    
    # Posterior CI
    postCI=quantile(effectSize,probs=c((1-ci)/2,ci+(1-ci)/2))
    ...
    # Text above the plot
    mtext(paste0(ci*100,"% ","CI: [", round(postCI[1], 3),",",round(postCI[2], 3),"]"), side=3,line=0.15,at=min(effectSize),col="black",cex=1,font=2)
    mtext(paste0("Post. Mean: ", round(mean(effectSize),3)),side=3,line=1.15,at=max(effectSize),col="black",cex=1,font=2)
    mtext(paste0("Post. Mode: ", round(getmode(effectSize),3)),side=3,line=0.15,at=max(effectSize),col="black",cex=1,font=2)
